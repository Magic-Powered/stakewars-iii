...
*/ -}}
{{- define "k8s-node.statefulsetSpec" -}}
{{- /*
...
kind: StatefulSet
metadata:
  name: {{ include "k8s-node.fullname" . }}{{ if .isCanary }}-canary{{ end }}
  labels:
    ...
    {{- range $key, $value := .Values.additionalStatefulSetLabels }}
    {{ $key }}: {{ $value }}
    {{- end}}
...
{{- end }}
spec:
  serviceName: {{ include "k8s-node.fullname" . }}
...
{{ toYaml .Values.additionalContainerEnv | indent 12 }}
        ...
        - name: {{ $key }}
          {{- if not (hasKey $value "containerImage") }}
          {{- $repo := $containerImage.repository }}
          {{- $tag := $containerImage.tag }}
          {{- $policy := $containerImage.policy }}
          image: "{{ $repo }}:{{ $tag }}"
          ...
          configMap:
            ...
            items:
              ...
              - key: {{ $configKey }}
                ...
                mode: {{ include "k8s-node.fileModeOctalToDecimal" $keyMountConfig.fileMode }}
                {{- end }}
              {{- end }}
            {{- end }}
      {{- end }}
...
{{ toYaml . | indent 8 }}
  ...
  {{- /* START VOLUME CLAIM TEMPLATES LOGIC */ -}}
...
{{ toYaml . | indent 4 }}
  ...
  {{- /* END VOLUME CLAIM TEMPLATES LOGIC */ -}}
{{- end -}}
